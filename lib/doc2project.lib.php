...
/**
 * @param OrderLine|PropaleLigne $line
 * @return array
 */
function getTasksForLine($line)
{
	global $conf, $db;
	$tab = array();
	...
	if ($resql)
	{
		...
		{
			$task = new Task($db);
			$taskid = ($obj->sourcetype == 'project_task') ? $obj->fk_source : $obj->fk_target;
			$ret = $task->fetch($taskid);
			if ($ret > 0)
			{
				$tab[] = $task->getNomUrl(1);
			}
		}
	}
